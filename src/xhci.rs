    ...
    async fn run(xhc: Controller) -> Result<()> {
        ...
        if let Some(port) = connected_port {
            ...
            let device_descriptor =
                Self::request_device_descriptor(&xhc, slot, &mut ctrl_ep_ring)
                    .await?;
            ...
            if let Ok(e) = Self::request_string_descriptor_zero(
                &xhc,
                slot,
                &mut ctrl_ep_ring,
            )
            .await
            {
                ...
                let mut boot_keyboard_interface: Option<InterfaceDescriptor> =
                    None;
                ...
                let config_desc =
                    last_config.ok_or("No USB KBD Boot config found")?;
                let interface_desc = boot_keyboard_interface
...
impl OperationalRegisters {
    ...
    fn set_dcbaa_ptr(
        &mut self,
        dcbaa: &mut DeviceContextBaseAddressArray,
    ) -> Result<()> {
...
impl Controller {
    ...
    fn init_primary_event_ring(&mut self) -> Result<()> {
        ...
        unsafe { self.regs.rt_regs.get_unchecked_mut() }
...
// DO NOT implement Copy trait - this should be the only instance to have the
// ptr.
pub struct Doorbell {
    ptr: Mutex<*mut u32>,
}
