            ...
            (UsbHidReportItemType::Main, 0b1000) => {
                ...
                if let Some(usage_page) = usage_page {
                    ...
                    for i in 0..report_count {
                        let report_usage =
                            if let Some(usage) = usage_queue.pop_front() {
                                usage
                            } else if let (
...
impl UsbHidReportInputItem {
    fn value_from_report(&self, report: &[u8]) -> Option<i64> {
        extract_bits_from_le_bytes(report, self.bit_offset, self.bit_size).map(
            |v| {
                if self.bit_size >= 2
                    && extract_bits(v, self.bit_size - 1, 1) == 1
                {
...
impl UsbTabletDriver {
    async fn run(
        ...
        let report_size_in_byte =
            if let Some(last_item) = input_report_items.last() {
        ...
        loop {
            ...
            let _ax =
                desc_abs_x.mapped_range_from_report(&report, 0..=(vw - 1));
            let _ay =
                desc_abs_y.mapped_range_from_report(&report, 0..=(vh - 1));
...
impl UsbDeviceDriver for UsbTabletDriver {
    ...
    fn start(
        ...
        spawn_global(async move {
            Self::run(&xhc, slot, &mut ctrl_ep_ring, &descriptors).await
        });
    }
}
