...
fn efi_main(image_handle: EfiHandle, efi_system_table: &EfiSystemTable) {
    ...
    let abp_uart_task = async {
        ...
        let base_addr = 0xfe032000 as usize; // chromebook boten/bookem
        let reg_rx_data = (base_addr + 0b000) as *mut u8;
        ...
        unsafe {
            ...
            write_volatile((base_addr + 0) as *mut u8, 1);
        ...
        loop {
            ...
            info!("----");
            let data = unsafe {
                read_volatile(reg_rx_data)
            };
            info!("DATA:      {data:#010X}");
            let status = unsafe {
                read_volatile(reg_line_status)
            };
